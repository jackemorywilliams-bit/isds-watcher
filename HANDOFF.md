...
| `icsid` | Case DB is JS-only; falls back to `/news-events` announcements. A zero-item run is probed: QUIET (page live, newest release dated) or NOT-READ — never the generic fetcher-mismatch alarm (added 2026-09-08 after a false DEGRADED on three same-day runs). |
...
**Ordered behind the mover (2026-09-10).** The `currency` job no longer lives in `pipeline-guards.yml`. It is now a second job inside `.github/workflows/reanchor.yml`, declaring `needs: reanchor`, so it cannot start until the re-anchor has landed. Before this, the two workflows fired on the same push with no ordering between them and the guard read the anchors while the mover was still computing the commit that fixes them — which is the whole explanation of the red-then-green flicker on every council merge. Its gate is `if: always() && (needs.reanchor.result == 'success' || needs.reanchor.result == 'skipped')`: `always()` is what lets it run when `reanchor` is *skipped*, which is what happens on a fork PR whose read-only token cannot push, and the result test is what stops it running when `reanchor` *failed*, where the anchors are in an unknown state. It checks out the head branch **by name** at `fetch-depth: 0` rather than the pull-request merge ref, because that merge ref was computed when the event fired and does not contain the `[skip ci]` commit `reanchor` just pushed; on the fork path, where that branch does not exist in this repository, the ref falls back to checkout's own default. The guard itself is untouched — same `scripts/check_currency.py`, same four predicates, same check-run name `currency`.

**What the ordering costs, stated because it is a real fail-open.** GitHub reports a job skipped by its own `if:` as **Success**, and `currency` is skipped by design whenever `reanchor` does not succeed. So a *failed* re-anchor now leaves a green `currency`, where the old independent job would have run and could have gone red. Nothing is armed by this today: checked 2026-09-10 during integrity review, `gh api repos/:owner/:repo/branches/main/protection` returns no `required_status_checks` and rulesets is `[]`, consistent with the deliberate no-status-check decision recorded under "Branch protection" above. **But if either check is ever made required on `main`, `reanchor` must be made required ALONGSIDE `currency`, never `currency` alone** — required alone, it is fail-open on exactly the path where the anchors are least trustworthy. There is nothing to *re-pin*: required status checks key on the check name, and the job id is `currency` in both the old and the new location with no `name:` override, so any existing pin keeps resolving without action.

**Two things for a session.** A red `currency` is now a real finding rather than the known false alarm, so do not wait it out and do not hand-write an anchor. And the old workaround — re-running `pipeline-guards.yml` on the branch to clear it — no longer applies, because the job is not in that workflow. The replacement is `gh workflow run reanchor.yml --ref <branch>`, and it is **not the same kind of action**: the old re-run was read-only, whereas this one **writes** — it runs `scripts/reanchor.py` and pushes a `[skip ci]` re-anchor commit to your branch. Expect a new commit, and `git pull` before you continue.
