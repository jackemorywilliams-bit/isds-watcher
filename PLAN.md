...
- **Hybrid digest sizing**: report every match at/above threshold (no upper cap), with a minimum of six items filled from the closest near-misses only down to a relevance floor of 25 (`MIN_DIGEST_ITEMS=6`, `RELEVANCE_FLOOR=25`). A quiet week may send only 0–3 items; a week with nothing above 25 sends a status note instead of padding. (2026-08-08: the fill is suspended by default pending validation — FILL_FLOOR_SUSPENDED — and the zero-state wording was replaced with a status note that does not assert the absence of developments.)
- **Item publication is gated entirely, as of 2026-08-09.** `VALIDATION_STATUS_ONLY` (`src/config.py:73-74`) defaults ON and holds **every** item-level entry — *including items at or above the threshold of 40* — together with the Research Brief, until production-path validation authorizes publication. It is a separate flag from `FILL_FLOOR_SUSPENDED` and neither can disable the other, so **restoring the fill would not reopen item publication**. A gated cycle sends one status note reporting the screening count and the number of items held. **The sizing bullet above therefore describes the selection machinery, not what the instrument currently sends: today it sends no items.**
- **Three capabilities exist behind default-off flags, as of 2026-08-09** — the ring-evidence contract of the validation record (`analytics/locked_set/VALIDATION_RECORD.md`; supersedes the uncommitted "R2.1 record", 2026-09-13) deriving in shadow (`src/rings.py`; `STATE_MODEL_V2="on"` refused until `STATE_MODEL_V2_PUBLICATION_READY`, which is `False`), a semantic classification path (`src/classify_v2.py` + `prompts/classifier_v2.txt`; `V2_SHADOW_CALLS` off, so every default-run verdict is labelled `lexical_only`), and a pre-enrichment semantic triage lane (`src/triage.py` + `prompts/triage.txt`; `TRIAGE_ENABLED=0`). The stratified tail audit from the same design is a **stub only** (`TAIL_AUDIT_N = 0`), expressly unimplemented.
