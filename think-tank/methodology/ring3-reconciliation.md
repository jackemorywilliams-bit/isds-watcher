...
*[Recorded 2026-08-08: the block above is harness output, not an instrument-level performance claim. See the correction appended at METHODOLOGY.md §VI.B — the exact CP 95% interval on 3/4 is 0.19–0.99, and the boundary the harness evaluates (40) has never fired in production.]*
...
**How the backtest confirms it.** Running `scripts/eval_holdout.py` on the unchanged scorer yields the same confusion matrix (TP 3, FP 0, TN 16, FN 1) over the 20-item out-of-sample set. Because the prose fix changes no weight, threshold, or trigger, that matrix is unchanged by the reconciliation. It is a descriptive record over four positives and grounds no performance claim, so nothing in this reconciliation rests on one. The holdout still illustrates (via the HIGH-scoring merits case and the missed jurisdiction-disposed case) that scoring tracks doctrinal engagement, not disposition — as an illustration, not a validation. *(Corrected 2026-08-08: this paragraph previously restated the four derived rates and called the empirical performance "undisturbed"; the rates are component-level descriptive figures with CP 95% intervals too wide to ground a performance claim — see METHODOLOGY.md §VI.B.)*
