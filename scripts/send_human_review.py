...
def main(argv=None) -> int:
    ...
    # The checks lead. The packet exists for the things only Emory can do; the
    # context that explains them follows. Previously they sat in section 4,
    # below everything else, which is why the packet read as undifferentiated.
    parts.append(f"## What needs you this week\n\n{review_md}\n\n---\n")

    council = _latest_weekly_council()
    parts.append(f"## This week's council roundtable\n\n{council['markdown']}\n")
    ...
    if sysres:
        parts.append(f"## Workflow improvement suggestions ({sysres[0]})\n\n{sysres[1]}\n\n---\n")
    else:
        opt = _optimization_snapshot()
        daily = _latest_record("daily-research")
        parts.append("## Workflow improvement suggestions\n\n")
        if opt:
    ...
    parts.append(f"## Where the research question stands\n\n{_state_of_answer_snapshot()}\n\n"
                 "Full detail: STATE_OF_THE_ANSWER.md in the repository.\n\n---\n")


    subject = f"ISDS Monday Review Packet — {date_str}"
    n_checks = draft.get("total", 0) if isinstance(draft, dict) else 0
    body_html = email_shell.render(
        "\n".join(parts),
        title="Monday Review Packet",
        kicker="ISDS Thematic Watcher",
        issue=date_str,
        lede=("Everything that needs <strong>your</strong> eyes this week — the checks "
              "only you can do come first, then the council's week, the instrument, and "
              "where the research question stands. This goes to you alone; "
              "Dr. Benavides's digest is sent separately."),
        footer=("Generated by the ISDS Thematic Watcher. Nothing in this packet is "
                "asserted to anyone until you verify it in the ledger."),
    )
