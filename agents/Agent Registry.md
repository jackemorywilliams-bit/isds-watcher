...
**Currency anchor.** *Audited against `465ce4e`.* Machine-owned; `scripts/reanchor.py` moves the sha to the session's last substantive commit in a notes-only close-out commit that `scripts/check_currency.py` excludes from drift as maintenance. Do not hand-edit the sha; the dated snapshot-anchor narrative below is preserved unedited as history.
...
| [[integrity-officer]] | **Taxonomy entry 30 — re-implementation drift.** A figure published as a function's output where the function was re-implemented rather than executed, and the re-implementation differs by one operation. Countermeasure: execute against a scratch copy and read the return | 2026-09-11 | `3f25b7f` (proposed, `2026-09-11.md:702`) / `28c0db8` (adopted, `:962`) |
| [[integrity-officer]] | **Taxonomy entry 31 — assumed-remedy claim.** Asserting a repair path *would* restore a record where the path was never executed. Countermeasure: run the remedy in a scratch copy and quote its output, or state it untested | 2026-09-11 | `3f25b7f` (proposed, `2026-09-11.md:704`) / `28c0db8` (adopted, `:962`) |
| [[council-chairman]] · [[research-analyst]] · [[integrity-officer]] · **every seat** | **Any claim in a daily record about what a code path does, or would do, carries that path's actual output — executed against a scratch copy and quoted — or is marked UNTESTED in the sentence that makes it.** The operative, practice-side form of entries 30 and 31, adopted as the sitting's Rule 0 optimization item and **expressly deduped** into `analytics/optimization-log.md` rather than added to it | 2026-09-11 | `28c0db8` / `2026-09-11.md:1006-1016` |
| [[integrity-officer]] | **Taxonomy entry 32 — single-window referent clause.** An Amendment 2 referent clause stating where a term's occurrences lie, written from a first-match-only instrument. Countermeasure: the clause names that limit and never states where the term does not occur | 2026-09-12 | `3828137` (proposed, `2026-09-12.md:1102`) / `639c16e` (adopted, `:1376`) |
| [[integrity-officer]] | **Taxonomy entry 33 — self-contradicting adjacent enumeration.** A count disagreeing with the enumeration in the adjacent sentence. Proposed as sub-number `17b/14a`, **adopted as its own entry** on three verified instances in one sitting | 2026-09-12 | `3828137` (proposed, `2026-09-12.md:1112`) / `639c16e` (adopted as 33, `:1377`) |
...
| [[integrity-officer]] | **Taxonomy entry 37 — cross-boundary window attribution.** A fixed-width window straddles a section boundary it does not mark, and content from after the boundary is attributed to the subject before it; the window is accurate and the *attribution* is the defect. **Binding countermeasure: a window containing a section heading is split at the heading, and no span after the heading is attributed to the subject of the span before it, unless a second independently-centred window carries the boundary's other side** | 2026-09-14 | `48d43ca` (proposed, `2026-09-14.md:623`) / `0390fe3` (adopted, `:769`; binding form `:941`) |
| [[council-chairman]] · [[research-analyst]] · [[integrity-officer]] | **A candidate literal is screened against the assembled on-state run of every committed window for that page — never against the list of spent finds, and never against a single window.** Adopted as the chairman's Ruling 1 of 2026-09-15, sustained against himself: row 1's window was wholly contained in the stitched union of two windows already committed on 2026-09-13 (overlap 282, union 518, containment `True`), so the row bought **zero** new characters while twenty-one correctly enumerated spent pairs reported it clear. The pair list is the wrong instrument for the occurrence test | 2026-09-15 | `b509f1d` / `analytics/daily-research/2026-09-15.md:1202-1211` |
| [[integrity-officer]] | **Taxonomy entry 38 — exact-match screen generalised to a pattern claim.** A screen by exact-URL or exact-string equality returns a correct answer *at that key*, and the result is then stated at the level of a path class, URL shape or spelling family — unscreened at the scope asserted while looking maximally screened. **Binding countermeasure: an absence claim about a class of locator is screened by the class predicate, never by equality on one member, and the sentence names the predicate it screened.** Proposed **by name only**, the proposing seat expressly declining to number it off a stale vault heading; the number was issued by the chairman | 2026-09-16 | `007909d` (proposed by name, `2026-09-16.md:669`) / `c231022` (number issued, `:1021`; binding form `:1129`) |
| [[integrity-officer]] | **Taxonomy entry 39 — dedup-screen referent inversion.** The required duplication screen runs, **returns the prior entry**, and the hit is characterised as immaterial rather than quoted; the finding is then filed as novel *and* the record reports the project as having missed what it recorded. Countermeasure: a dedup screen is discharged by quoting the headline of every line it returned, never by characterising them. **Numbered by the proposing seat off a heading it verified was current** — the one sitting in this window on which D15 did not fire. | 2026-09-17 | `analytics/daily-research/2026-09-17.md:887` (proposed, `226d697`), `:1275` (adopted, `710842c`) |
| [[integrity-officer]] | **Taxonomy entry 40 — stale-population screen record.** A screen's population figure is carried from an earlier session and restated as counted at the commit the screen names, while the hit counts are genuinely recomputed; the hits being right is what makes it invisible. Countermeasure, in the analyst's tightened and adopted form: the population is printed by the pass that produces the hits, in the same output, adjacent to them. Proposed **by name only**. | 2026-09-18 | `analytics/daily-research/2026-09-18.md:906` (proposed by name, `af479da`), `:1195` (adopted, `018c1f1`) |
| [[integrity-officer]] | **Taxonomy 34a — licence-in-the-pre-registration.** A pre-registration states what an outcome *"decisively"* establishes at a strength the row cannot carry; because the note is never edited, the overstatement is frozen and can only be withdrawn. Adopted **as an extension of 34, not a new number**, expressly because of this table's collision history — the second time that precedent has governed. Proposed **by name only**. | 2026-09-18 | `analytics/daily-research/2026-09-18.md:904` (proposed by name, `af479da`), `:1203` (adopted as an extension, `018c1f1`) |
| [[council-chairman]] · [[research-analyst]] · [[integrity-officer]] · **every seat** | **A screen's population is printed by the pass that produces its hits, in the same output, adjacent to them — never re-derived in a second command and never carried between sessions.** Entry 40 stated as a binding writing discipline, tightened by the analyst from "re-derived in the same session" because *"a count re-derived in a second command is a second observation of a tree that may have moved."* Corroborated the same day by three seats producing three numbers for one screen while all three were silent about the unit. | 2026-09-18 | `analytics/daily-research/2026-09-18.md:1003` (tightening), `:1191-1199` (adopted in the tightened form, `018c1f1`) |
| [[council-chairman]] · [[research-analyst]] | **Nothing contested, and no "decisive", goes into a pre-registration** — a pre-registered licence is written at the weakest form that still makes the row a test, because a falsification condition does its work by being pre-committed, not by being strong. Extension 34a stated as a binding writing discipline; the chairman recorded the instance against himself as much as against the analyst. | 2026-09-18 | `analytics/daily-research/2026-09-18.md:1339` (binding form, `018c1f1`) |
| [[integrity-officer]] · [[research-analyst]] · **every seat** | **An occurrence count offered in support of a proposition excludes loci whose content is a screen record for the same term, and says how many were excluded** — *zero-screen counted as a carrier*, **adopted as binding practice and deliberately left unnumbered**, the entry-27 collision being cited as the reason no number was taken. The record's report of a term's absence otherwise becomes, to a grep, the term's presence. It fired on the analyst the day it was named. | 2026-09-19 | `analytics/daily-research/2026-09-19.md:879` (proposed by name, `b267efd`), `:1323-1326` (adopted as binding practice, unnumbered) |
| [[integrity-officer]] | **Taxonomy entry 41 — invented limb.** A seat attributes to a cited internal authority a proposition that authority does not contain — typically by imposing a "first limb / second limb" structure on a document that has neither — and reports the manufactured limb as contradicted or in doubt. The locator is correct and checkable, which is what makes it pass. **Binding countermeasure, in the chairman's adopted form with the analyst's amendment: a sentence putting a cited internal authority in doubt quotes verbatim, at a pinpoint, the LIMB, PREMISE, GROUND, PRONG OR BRANCH it contests, before contesting it.** The widening beyond *limb* is load-bearing — the generating instance used the word *premise*, so an entry keyed to *limb* would have been evaded by its own instance. **Numbered by the proposing seat off a heading it opened and verified current.** | 2026-09-20 | `analytics/daily-research/2026-09-20.md:991` (proposed, `1d6e3e7`), `:1238` (amendment), `:1315` (adopted, `1d6e3e7`), `:1508` (binding form) |
| [[integrity-officer]] | **Taxonomy 40a — unit-swapped screen record.** A screen publishes **line** counts under an explicit "occurrences" label, or the reverse; population and scope are correct and only the unit is misdeclared, so the figure is unfalsifiable from the page. Adopted **as an extension of 40, not a new number** — the **third** time the entry-27 collision history has governed that choice, after 32a and 34a. **Binding countermeasure, as the analyst amended it: a screen record names its unit and its flag in the same clause as its number, exclusion arithmetic is done in the unit it was measured in, and where a screen restates figures on which a prior sustained correction depends it states expressly whether that correction is unchanged.** The amendment's ground, which the chairman called the sharper half of the entry: a unit swap **can silently retire a correction the council has already sustained** and so read as substantive news. | 2026-09-20 | `analytics/daily-research/2026-09-20.md:995` (proposed as an extension, `1d6e3e7`), `:1240` (amendment), `:1323` (adopted, `1d6e3e7`), `:1508` (binding form) |
| [[council-chairman]] · [[research-analyst]] · [[integrity-officer]] · **every seat** | **Three procedures, proposed by the analyst against itself and adopted by the chairman.** (a) **Every string inside a `supporting_quote` or a `Q` mark is pasted back into the named file and its occurrence count printed before filing.** (b) **Every nonzero cell in a dedup screen has its lines enumerated by locator with their headline, never characterised** — entry 39's countermeasure, now mandatory rather than advisory. (c) **Every count in prose is computed in the session or deleted.** The chairman recorded what (c) alone would have caught the same day: *"eleven-word", written four times and counted never.* | 2026-09-21 | `analytics/daily-research/2026-09-21.md:1353-1359` (`da5cb53`; close-out landed on `main` at `7703720`, the merge commit for #208 — written without the `sha, PR #n` token for the reason given in [[Workflow Threads]] **D21**) |
...
- **2026-09-22 (archivist session)** — **No roster change and no contract change. Six rows added
  ...
  to five.** Measured rather than assumed: `git log f1a7850..1cf6108 -- .claude/agents/ prompts/
  src/models.py` returns **zero** commits across a **70-commit** window, so **no seat's contract
  was edited by anyone for the second consecutive session**; `scripts/check_models.py` exits **0**
  over 12 cards, `scripts/check_currency.py` exits **0** over 9 claims, and
  `node tools/isds-workflow-3d/validate.mjs` exits **0** over 30 cards, 10 chips and 44 edges.
  What moved is again only what the council adopted and could not file: **taxonomy 41** and
  **extension 40a**, both 2026-09-20 (`1d6e3e7`), plus two further patterns proposed by name and
  left unnumbered (2026-09-20 `:1334`; 2026-09-22 `:946`, `a814d18`), and the four procedural
  rules adopted on 2026-09-21 and 2026-09-22.
  **The model split is unchanged and re-enumerated from the files rather than carried:** **4**
  descriptions say Opus 5 (council-chairman, research-analyst, site-experience, systems-designer),
  **5** say Opus 4.8 (analytics-officer, integrity-officer, obsidian-archivist, research-editor,
  systems-researcher), **0** name no model; `site-experience.md` and `systems-designer.md` still
  carry no `- Model:` line in their DISCIPLINE blocks, and both say Opus 5, so nothing
  contradicts. Counted this session under the discipline of taxonomy entry 40.
  *Audited against `1cf6108`; paths: `.claude/agents/`, `prompts/`, `src/models.py`, `agents/`,
  `views/isds-workflow-3d/workflow.json`.*

- **2026-09-19 (archivist session)** — **No roster change and no contract change. Six rows added
  to the adopted-method-rules table; the taxonomy heading in [[integrity-officer]] moved 38 → 40
  and extension `34a` was filed with its parent.** Measured, not assumed: `git log
  98dcff9..f1a7850 -- .claude/agents/ prompts/ src/models.py` returns **zero** commits across an
  **83-commit** window, so no seat's contract was edited by anyone; `scripts/check_models.py`
  exits **0** over 12 cards and `scripts/check_currency.py` exits **0** over 9 claims. What moved
  is again what the council adopted and could not file: **taxonomy 39** (2026-09-17, `710842c`),
  **taxonomy 40** and **extension 34a** (both 2026-09-18, `018c1f1`) — **D15** at its ninth, tenth
  and eleventh consecutive occurrences. **Three further patterns proposed by name on 2026-09-19
  (`b267efd`) are filed unnumbered in a pending block**, one of them adopted as binding practice.
  **The countermeasures were re-read against the seat that is bound by them and the seat is
  handling the latency three different correct ways**: it verified the heading and numbered off it
  on 09-17, and declined the number and proposed by name on 09-18 and 09-19.
  **One correction entered against the bound seat's own self-report, in its note**: its 2026-09-19
  statement that *"my own definition still enumerates five patterns"* is **false against the file**
  — `.claude/agents/integrity-officer.md:11-21` names the four *contract categories*, not taxonomy
  entries, and `:48-56` forbids enumerating the taxonomy there at all. The read path is correct as
  written and needs no edit.
  *Audited against `f1a7850`; paths: `.claude/agents/`, `prompts/`, `src/models.py`, `agents/`,
  `analytics/daily-research/`, `METHODOLOGY.md`, `README.md`, `docs/`,
  `scripts/site_templates/`, `views/isds-workflow-3d/workflow.json`, and every remote branch tip.*
- **2026-09-16 (archivist session)** — **No roster change and no contract change. Three rows added
  to the adopted-method-rules table; the taxonomy heading in [[integrity-officer]] moved 36 → 38.**
  Measured, not assumed: `git log 349ca12..98dcff9 -- .claude/agents/ prompts/ src/models.py`
  returns **zero** commits across a 152-commit window, so no seat's contract was edited by anyone.
  `scripts/check_models.py` exits **0** over 12 cards. What moved is what the council adopted and
  could not file: **taxonomy 37** (2026-09-14, `0390fe3`), the **assembled-run screen** ruling
  (2026-09-15, `b509f1d`) and **taxonomy 38** (2026-09-16, `c231022`). All three reached a note for
  the first time today — **D15** at its seventh and eighth consecutive occurrence. The officer
  caught its own table's staleness on both days and declined to number off it
  (`2026-09-15.md:891`; `2026-09-16.md:671`), which is the mandate working and is also the proof
  that the file it is told to read was the thing that was late.
