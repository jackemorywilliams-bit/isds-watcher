...
def notable_quote(text: str, max_len: int = 240) -> str:
    """Return the sentence with the most doctrine-keyword hits (a 'notable
    one-liner'). Falls back to the first substantive sentence."""
    if not text:
        return ""
    kws = _doctrine_keywords()
    best, best_score = "", -1
    for sent in _split_sentences(text):
        if not (40 <= len(sent) <= max_len + 80):
            continue
        low = sent.lower()
        score = sum(2 for k in kws if k in low)
        # Mild bonus for evaluative/controversial language.
        score += sum(1 for w in ("egregious", "manifest", "abuse", "shock",
                                  "unjust", "arbitrary", "denial", "wrongful",
                                  "unprecedented", "controvers") if w in low)
        if score > best_score:
            best, best_score = sent, score
    if best_score <= 0:
        sents = _split_sentences(text)
        best = next((s for s in sents if len(s) >= 60), sents[0] if sents else "")
    if len(best) > max_len:
