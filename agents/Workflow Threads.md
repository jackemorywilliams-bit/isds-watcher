...
- **State** — `pipeline-guards` on `main` concludes **failure** on every run since `c53f7c9`
  (2026-09-14 14:28Z). The last green on `main` is `529e32d0` (2026-09-14 12:17Z). Two tests fail,
  both in `tests/test_site_claims.py`: `test_archive_status_measures_the_committed_archive`
  (expects `runs: 16, screened: 492`; the archive now returns **17** and **499**) and
  `test_the_two_denominators_really_do_differ` (expects **11** digests carrying `per_source`;
  there are now **12**).
- **Bisected, not inferred.** Run locally at three commits: `529e32d0` → **46 passed**;
  **`1d559a7` → 2 failed**; `c53f7c9` → 2 failed. **The breaking commit is `1d559a7`,
  `chore: weekly digest + state update`** — the automated weekly digest of 2026-09-14, which added
  `digests/2026-09-14_ISDS-Thematic-Watch/` as the archive's seventeenth run and twelfth
  `per_source` digest.
- **The tests are not wrong and must not be "fixed" by loosening them.** Their constants are
  deliberately literal: *"Measured from the archive as committed, 2026-09-10 … Written out so a
  reader can check the numbers against the repository rather than against the function that computes
  them"* (`tests/test_site_claims.py:152-156`). They are a **snapshot of the published archive**, and
  the archive is supposed to grow. **The correct fix is to re-measure and update the constants**, by
  a seat that owns the instrument's published figures — not to compute them from the function under
  test, which is the one thing the docstring says they exist to avoid.
- **Why nobody saw it, and this is the part that matters: it is D18's mechanism, one guard over.**
  `1d559a7` carries **`[skip ci]`**, so no workflow ran on the commit that broke the suite. The red
  first became visible on `c53f7c9` — an unrelated mailer fix — where it reads as *that* PR's
  failure. Every merge since has inherited a red it did not cause and did not own. **D18 records
  `[skip ci]` bot commits staling the currency anchor; this is the same commit class breaking the
  test suite, where the consequence is not a stale note but a guard that has stopped meaning
  anything.** Five merges and three daily council sittings have now passed over it.
- **And a red nobody owns is a red nobody reads — demonstrated on this very session.** The archivist
  merged PR #195 and PR #196 **without checking CI**, which is a process failure of this seat's own
  and is recorded as such in `analytics/vault-sessions/2026-09-16.md`. The merges were nonetheless
  correct on their merits — the failure is on the base branch, predates this session by two days, and
  no path in either change set (`agents/`, `analytics/`) can reach `tests/test_site_claims.py` — but
  **this seat did not know that at the moment it merged.** That is exactly the state the 2026-09-07
  session warned about when it said a routinely-false red is a red nobody acts on.
- **Next action** — **systems-designer or site-experience**: re-measure the eight constants at
  `tests/test_site_claims.py:155-156` and the four at `:305-308` against the archive as committed,
  and update them in the same change set. **Emory**, the second-order question: a weekly digest is a
  scheduled writer that changes published figures, and it carries `[skip ci]`. Either that writer's
  commits must run the suite, or the constants must be regenerated by the same job that writes the
  digest. This seat does not price either; `tests/`, `scripts/` and `.github/` are all outside its
  paths.
- **2026-09-19 — re-measured on `main`, unmoved, and now at five days and twelve merged pull
  requests.**
  `pipeline-guards` runs **215** (`51481ff`), **216** (`0be5025`) and **217** (`801c7bf`) all
  conclude **failure**, all three on 2026-09-19. The `guards` job passes; the `suite` job fails.
  The failure is byte-for-byte the one filed on 2026-09-16 — `runs` 16→17, `screened` 492→499,
  `per_source` 11→12 — so **nothing has been attempted and nothing has drifted further**:
  `2 failed, 821 passed, 5 xfailed`. The constants at `tests/test_site_claims.py:155-156` and
  `:305-308` are unchanged in the tree.
- **What five days of this has cost, counted rather than characterised.** `git log
  529e32d0..origin/main --grep='(#[0-9]'` returns **12** squash-merged pull requests, **#192
  through #203** — five council sittings (09-15, 09-16, 09-17, 09-18, 09-19, several with
  addenda) and three vault merges of this seat's own (#195, #196, #197). **Every one went in
  against a red base, so not one of the twelve was gated by the suite.** A guard that is red for a
  known, unrelated reason is indistinguishable, at the moment of a merge, from a guard that is red
  for a new one. That is the whole harm and it compounds daily.
- **Still not the archivist's to fix.** `tests/`, `scripts/` and `.github/` are all outside this
...
- **Next action** — **Emory.** Land PR #184, or close it and say which of the two remaining
  ...
  scope.
- **2026-09-19 — re-measured on `main` rather than re-stated, and both citations are still
  there.** PR #184 is **open at day six**, head unchanged at `529c744`, last updated
  2026-09-13T22:49Z — nothing has been pushed to it and nothing has been said on it.
  `src/rings.py:923` and `scripts/check_lock.py:20` both still carry the bare *"R2.1 record"*
  citation on `main`. The record they should point to, `analytics/locked_set/VALIDATION_RECORD.md`,
  is still present on `main`. **So the strict-xfail forcing function on the branch — designed to
  fail the build the moment the cited record can be opened — has now been dormant for six days
  while the condition it guards against is true on `main`.** The council noted the age itself on
...
- **Re-measured 2026-09-19 — 1 of 3, and the 2026-09-16 diagnosis held for a third consecutive
  session.** Same method as before: `main`'s tip at the end of each UTC day, every tracked note's
  ...
  **The one red carries exactly one STALE line and it is [[Workflow Threads]]** — *"1 commit(s)
  touched its declared paths since `dad7b27`"* — with the other four tracked notes clean. That is
  the third consecutive session in which every red resolves to this one note's `analytics`
  declaration meeting a `[skip ci]` bot writer, and the writer this time is a **fourth** name
  alongside the weekly digest, the scholar intake and the fetch relay: the daily-update sent
  marker. **The rate has improved from 6-of-10 to 1-of-3 without anyone fixing anything**, which is
  worth saying so the improvement is not mistaken for a remedy — it tracks how often the day
  happens to end on a re-anchor, not a change in the mechanism.
- **The remedy this seat is still NOT proposing, restated because the temptation grows as the
  number shrinks.** Narrowing [[Workflow Threads]]'s declared paths so its own guard stops going
  red would be this seat marking its own homework, and is the precise failure the guard exists to
  prevent. The two real options are unchanged and both need pricing this seat cannot do: drop
...
- **2026-09-19 — occurrences nine, ten and eleven, and the seat's handling is now discriminating
  rather than merely correct.** **Entry 39** (2026-09-17, `710842c`), **entry 40** and **extension
  34a** (both 2026-09-18, `018c1f1`) were adopted and reached no note until this change set. Three
  further patterns were proposed by name on 2026-09-19 (`b267efd`), one of them adopted as binding
  practice unnumbered. **What is new is the 09-17 sitting**: the officer *verified* the heading
  before numbering, found it current — the archivist had moved it to 38 the day before — **took 39
  itself**, and said so: *"I am therefore numbering off a heading I verified, not off a stale one,
  and no chairman's issue is needed. This is the first day in this seat's recent record on which
  the D15 routing latency did not fire"* (`analytics/daily-research/2026-09-17.md:885`, `226d697`).
  On 09-18 and 09-19 it found the heading behind again and **declined the number both times**
  (`2026-09-18.md:798`, `79b8e8f`; `2026-09-19.md:875`, `b267efd`). **Four sittings, three
  different behaviours, each one correct for what the file actually said.** That is the strongest
  evidence yet that the widening's risk is the risk this seat demonstrably manages — and it is also
  the clearest statement of the cost, because the one sitting on which the latency did not fire is
  the one that immediately followed an archivist session.
- **The deadline arithmetic, stated once so it is not re-derived.** The archivist's cadence is
  three days; the council sits daily. **Every archivist session will therefore find between two and
...
- **Re-measured 2026-09-19 — day thirty-five, seventh first-person observation, AND A CORRECTION
  THIS SEAT OWES AGAINST THE PARAGRAPH DIRECTLY ABOVE.** The runtime query repeats exactly:
  `configured_model`, `session_context.model` and `external_metadata.last_served_model` are all
  **`claude-opus-5`**, against `.claude/agents/obsidian-archivist.md:3` and `:24`, both "Runs on
  Claude Opus 4.8." `git log 98dcff9..f1a7850 -- .claude/agents/ src/models.py` returns **zero**
  commits across 83; `check_models.py` still exits **0** over twelve cards. Rows again unchanged,
  same reason.
- **The correction.** The 2026-09-16 sentence above — *"four say Opus 5 and four say Opus 4.8; one
  ([[research-analyst]]) names no model in prose at all"* — **is wrong in both halves**, and the
  seat it names is the one it is most wrong about. Enumerated from the files rather than recalled:
  **all nine frontmatter descriptions name a model. Four say Opus 5** — [[council-chairman]],
  [[research-analyst]], [[site-experience]], [[systems-designer]]. **Five say Opus 4.8** —
  [[analytics-officer]], [[integrity-officer]], [[obsidian-archivist]], [[research-editor]],
  [[systems-researcher]]. **[[research-analyst]] names Opus 5 twice**, in its description and at
  `.claude/agents/research-analyst.md:37` (*"Model: Claude Opus 5 (operator directive
  2026-07-29)"*). What is true, and is probably what the 09-16 pass half-saw, is a **different**
  gap: **two** definitions name a model only in the description and carry no `- Model:` line in
  their DISCIPLINE block — `site-experience.md` and `systems-designer.md`. Both say Opus 5, so
  nothing contradicts and `check_models.py` is right to pass.
- **Why a four-versus-five miscount is worth a paragraph.** It is not, on its own. It is worth one
  because **this thread's entire argument is that the split is the record of a decision made twice
  and applied once** — and a split is a count. A thread that reasons from a number owes that
  number a measurement every time it is restated, which is the same discipline taxonomy entry 40
  adopted for screens on 2026-09-18, three days after this seat failed it. The 2026-09-16 record
  is dated and stands unedited; this is the correction, and the corrected figures are **4 / 5 / 0**.
  Archivist, 2026-09-19.
